...
#import "InvSchCom.h"
#import "InvSchComTableViewCell.h"
#import "AbsViewController.h"
#import <SDWebImage/UIImageView+WebCache.h>
#import "Constant.h"
@interface InvSchViewController ()

@end

@implementation InvSchViewController
...
- (void)viewDidLoad {
    ...
    //self.images.layer.cornerRadius=40;
    ...
    NSLog(@"****Session  ID invited ==%@",_tempsessionid);
    
    [[NSUserDefaults standardUserDefaults]setValue:_tempsessionid forKey:@"sessionid"];
...
{
    if([[segue identifier] isEqualToString:@"showInvitedSchdescAbs"])
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    InvSchComTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
    
    InvSchCom *ktemp=[_keynotecommentarr objectAtIndex:indexPath.row];
    ...
    NSURLSessionDataTask *task=[defaultSession dataTaskWithRequest:urlRequest completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        for(NSDictionary *temp in keyarr)
                                        {
                                            InvSchCom *f=[[InvSchCom alloc]init];
                                            f.name=temp[@"user"];
...
-(void)getCommentCount{
    ...
    NSString *savedsessionid=[[NSUserDefaults standardUserDefaults]stringForKey:@"invitedid"];
    
    NSString *myst2=[NSString stringWithFormat:@"invited_id=%@",savedsessionid];
...
-(void)deleteSession
{
    ...
    NSString *myst=[NSString stringWithFormat:@"emailid=%@&invited_id=%@&session_id=%@",savedValue,savedinvitedid,sessionid];
    NSLog(@"+++my string after delete in inv sch detailss=%@",myst);
