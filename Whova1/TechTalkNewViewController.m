...
#import "TechTalkTableViewCell.h"
#import "AbsViewController.h"
#import "PanelCommentTableViewCell.h"
#import "PanelPanelistViewController.h"
#import "PanelComment.h"
#import "Panel.h"
#import "Constant.h"
@interface TechTalkNewViewController ()

@end

@implementation TechTalkNewViewController
    ...
    NSURLSessionDataTask *task4=[defaultSession dataTaskWithRequest:urlRequest completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error)
                                 {
                                     ...
                                     else
                                     {
                                         ...
                                         for(NSDictionary *temp in arr){
                                             PanelComment *f=[[PanelComment alloc]init];
                                             f.name=temp[@"user"];
...
- (IBAction)likeBtnClicked:(id)sender {
    ...
    if (emailid==NULL) {
        UIAlertController * alert=[UIAlertController
                                   ...
                                   alertControllerWithTitle:@"Alert!" message:@"Please login to like panel session"preferredStyle:UIAlertControllerStyleAlert];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView==self.tableview) {
        ...
        TechTalkTableViewCell *cell=[_tableview dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
        
        Panel *w=[_panelistarr objectAtIndex:indexPath.row];
        ...
        PanelCommentTableViewCell *cell = [self.tableview2 dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
        
        PanelComment *ktemp=[_commentarray objectAtIndex:indexPath.row];
...
{
  if([[segue identifier] isEqualToString:@"showPanelabstract"])
  {
      AbsViewController *ab=[segue destinationViewController];
      ab.absStr=_abstract.text;
  }
    
    if([[segue identifier] isEqualToString:@"showPanelist"]){
        
        PanelPanelistViewController
        *ab=[segue destinationViewController];
        ab.tempsessionid=self.sessionid.text;
        
        NSLog(@"sessionid==%@",ab.tempsessionid);
...
-(void)addSession{
    ...
    NSURL * url = [NSURL URLWithString:[mainUrl stringByAppendingString:add_to_schedule]];
