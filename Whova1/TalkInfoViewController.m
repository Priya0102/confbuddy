...
- (void)viewDidLoad {
    ...
    self.sessionid.text=_tempsessionid;
    
    NSLog(@"locaton talk in session==%@ SESSION ID-->%@",_locationstr,_tempsessionid);
    
    [[NSUserDefaults standardUserDefaults]setValue:self.talkidstr forKey:@"talk_id"];
    NSLog(@"talk_id in session= %@", [[NSUserDefaults standardUserDefaults] objectForKey:@"talk_id"]);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TalkCommentTableViewCell *cell = [self.tableview dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
    
    TalkComment *ktemp=[_keynotecommentarr objectAtIndex:indexPath.row];
...
-(void)keynoteparsing
{
    ...
    //NSURL * url = [NSURL URLWithString:@"http://localhost/phpmyadmin/itherm/talk_info.php"];//local
    ...
    NSURLSessionDataTask *task4=[defaultSession dataTaskWithRequest:urlRequest completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error)
                                 {
                                     ...
                                     else
                                     {
                                         ...
                                         for(NSDictionary *temp in arr){
                                             TalkComment *f=[[TalkComment alloc]init];
                                             f.name=temp[@"user"];
...
-(void)addSession{
    ...
    NSURL * url = [NSURL URLWithString:[mainUrl stringByAppendingString:techtalk_reminder_add]];
...
- (IBAction)likeBtnClicked:(id)sender {
    ...
    if (emailid==NULL) {
        UIAlertController * alert=[UIAlertController
                                   ...
                                   alertControllerWithTitle:@"Alert!" message:@"Please login to like session"preferredStyle:UIAlertControllerStyleAlert];
...
-(void)getTalkDetailStatus{
    ...
    NSLog(@"Get talk details status SAtatement...***%@",myst);
    
    NSURL * url = [NSURL URLWithString:[mainUrl stringByAppendingString:get_talk_details_status]];
    
    //NSURL * url = [NSURL URLWithString:@"http://192.168.1.100/phpmyadmin/itherm/get_talk_details_status.php"];//local
    ...
    NSURLSessionDataTask *task=[session dataTaskWithRequest:urlrequest completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error)
                                {
                                    ...
                                    NSLog(@"json talk add to myschedule = %@",responsedict);
...
/*-(void)addSession1
 ...
 //NSURL * url = [NSURL URLWithString:@"http://192.168.1.100/phpmyadmin/itherm/add_to_talk_schedule.php"];
